...
export function checkForCollisionsAgainstEnvironment(actor) {
  ...
  const isCollidingWithPlatform =
    isPlatform(verticalCollisionMarkerLeft, bottom) ||
    isPlatform(verticalCollisionMarkerRight, bottom);
  const adjustPlatformPosition =
    !isSolid(
      verticalCollisionMarkerLeft,
      roundToNearestCell(actor.previousY - height || 0),
    ) ||
    !isSolid(
      verticalCollisionMarkerRight,
      roundToNearestCell(actor.previousY - height || 0),
    );
